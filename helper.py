...
def computeBrief(img, locs):
	...
	locs = np.array(list(filter(lambda x: halfWidth <= x[0] < m-halfWidth \
     and halfWidth <= x[1] < n-halfWidth, locs)))
	desc = np.array([list(map(lambda x: computePixel(img, x[0], x[1], patchWidth, c), zip(compareX, compareY))) \
     for c in locs])
